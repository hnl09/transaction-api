...
describe('Transaction Entity', () => {
  ...
  it('should create a transaction successfully with valid data', () => {
    ...
    expect(transaction.timestamp.toISOString()).toBe(validTimestamp.toISOString());
  });
  ...
  it('should throw an error if amount is negative', () => {
    expect(() => new TransactionEntity(-10, validTimestamp))
      .toThrow('Transaction amount must be a non-negative number.');
  });

  it('should throw an error if timestamp is in the future', () => {
    const futureTimestamp = new Date(Date.now() + 100000);
    expect(() => new TransactionEntity(validAmount, futureTimestamp))
      .toThrow('Transaction timestamp cannot be in the future.');
  });

  it('should throw an error for an invalid timestamp string', () => {
    expect(() => new TransactionEntity(validAmount, 'invalid-date-string'))
