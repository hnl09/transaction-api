...
describe('CreateTransactionUseCase', () => {
  ...
  it('should create a transaction and call repository.create', async () => {
    const command = new CreateTransactionCommand(100.50, new Date(Date.now() - 5000).toISOString());
    const mockCreatedTransaction = new TransactionEntity(command.amount, command.timestamp);
    ...
    expect(repository.create).toHaveBeenCalledWith(expect.any(TransactionEntity));
    const calledWithArg = mockTransactionRepository.create.mock.calls[0][0] as TransactionEntity;
  ...
  it('should throw UnprocessableEntityException if timestamp is in the future', async () => {
    const command = new CreateTransactionCommand(50, new Date(Date.now() + 100000).toISOString());

    await expect(useCase.execute(command))
      .rejects.toThrow(UnprocessableEntityException);
    await expect(useCase.execute(command))
      .rejects.toThrow('Transaction timestamp cannot be in the future.');
